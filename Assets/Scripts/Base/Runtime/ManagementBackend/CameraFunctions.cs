...
namespace Base {
    ...
    [Serializable]
    public class CameraFunctions : B_ManagerBase {
        ...
        public override Task ManagerStrapping() {
            ...
            B_CentralEventSystem.OnLevelDisable.AddFunction(FlushData, true);
        ...
        public void VirtualCameraShake(ActiveVirtualCameras Camera, float Amp, float TimeToShake) {
            B_CoroutineControl.Queue.RunCoroutine(Shaker(Camera, Amp, TimeToShake),VirtualCameras[Camera].coroutine);
        }

        public void ChangeCameraFOW(ActiveVirtualCameras Camera, float To, float Speed) {
            var Cam = VirtualCameras[Camera].VirtualCamera;
            B_CoroutineControl.Queue.EnqueueAction(Ieum_ChangeCameraFOW(Cam, To, Speed));
        }
